export const ROMANI_SOURCE = {
  book: "Romani",
  scripture: {
    translation: "Cornilescu, ediția corectată (RCCV)",
    source:
      "https://github.com/seven1m/open-bibles/blob/f257a3559025c3f873b48a75019f53a9354ed7de/ron-rccv.usfx.xml",
    sourceSha256: "3849c8be03a39ab4042408f8589895e24ee5ac36967054e63e692fb2f9e2c591",
    importConfig: "docs/data/romani-rccv-import.json",
    generatedAtBuild: true,
    publicLaunchRights: "unresolved",
  },
  poonen: {
    officialThroughTheBible: "https://www.cfcindia.com/books/through-the-bible",
    officialVerseByVerse: "https://www.cfcindia.com/verse-by-verse/Romans",
    manifest: "docs/data/romani-poonen-source.json",
    verifiedEpisodeCount: 15,
    controllingTheme: "Evanghelia care conduce la ascultarea credinței",
    themes: [
      "vinovăția universală și justificarea prin har",
      "unirea cu moartea și învierea lui Hristos",
      "eliberarea de legalism și viața în Duhul",
      "suveranitatea, dreptatea și credincioșia lui Dumnezeu",
      "consacrarea trupului și înnoirea minții",
      "unitatea Bisericii și primirea reciprocă",
      "misiunea printre neamuri",
      "ascultarea credinței și victoria asupra celui rău",
    ],
  },
  editorial: {
    status: "in_review",
    rawTranscriptCommitted: false,
    policy:
      "Explicația este redactată în română pe baza studiilor verse-by-verse și a transcrierilor Zac Poonen, păstrând fidel sensul doctrinar și firul explicației, fără traducere sau copiere 1:1 și fără interpretări doctrinare inventate de Emanus.",
  },
} as const
