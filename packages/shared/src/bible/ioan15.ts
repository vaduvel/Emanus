import type { BibleChapter } from "./types.js"

/*
 * Ioan 15, explicata verset cu verset.
 * Text: Cornilescu 1924, editia originala. Explicatia: scrisa pentru Emanus.
 */

export const IOAN_15: BibleChapter = {
  id: "ioan-15",
  bookId: "ioan",
  number: 15,
  title: "Ioan 15 — Vita adevărată, rămânerea, rodul și ura lumii",
  summary:
    "Domnul Isus Se descoperă ca adevărata Viță, cheamă la rămânerea în El pentru rodire, leagă porunca dragostei de jertfa Sa, avertizează asupra urii lumii și promite mărturia Duhului adevărului și a ucenicilor.",
  literaryContext:
    "Ioan 15 continuă vorbirea din odaia de sus și duce mai departe tema locuirii și a rămânerii. După făgăduința Mângâietorului și a păcii, Domnul cheamă acum la viața roditoare în El și pregătește ucenicii pentru prigonirea care vine. Capitolul se mișcă de la Vița și mlădițele, la dragostea poruncită între frați, apoi la contrastul dintre ucenici și lume. Astfel, intimitatea părtășiei cu Hristos nu este despărțită de realismul conflictului cu lumea.",
  historicalContext:
    "Imaginea viei era bine cunoscută în Scriptură și adesea aplicată lui Israel. Când Domnul spune: Eu sunt adevărata Viță, El Se așază ca împlinire a ceea ce Israel n-a putut purta deplin. În plus, avertismentul despre ura lumii vine către oameni care urmează să intre în opoziție deschisă cu sinagoga și cu puterile timpului. Mărturia Duhului și a ucenicilor devine astfel vitală pentru mersul Bisericii de după plecarea Domnului.",
  status: "in_review",
  units: [
    {
      id: "ioan-15-1-11",
      ref: "Ioan 15:1-11",
      heading: "Rămâneți în Mine",
      text:
        "Eu sunt adevărata Viță, și Tatăl Meu este Vierul. Pe orice mlădiță care este în Mine și n-aduce rod, El o taie; și pe orice mlădiță care aduce rod, o curăță, ca să aducă și mai mult rod. Acum voi sunteți curați, din pricina Cuvântului pe care vi l-am spus. Rămâneți în Mine, și Eu voi rămâne în voi. După cum mlădița nu poate aduce rod de la sine, dacă nu rămâne în viță, tot așa nici voi nu puteți aduce rod, dacă nu rămâneți în Mine. Eu sunt Vița, voi sunteți mlădițele. Cine rămâne în Mine și în cine rămân Eu aduce mult rod; căci despărțiți de Mine nu puteți face nimic. Dacă nu rămâne cineva în Mine, este aruncat afară, ca mlădița neroditoare, și se usucă; apoi mlădițele uscate sunt strânse, aruncate în foc și ard. Dacă rămâneți în Mine și dacă rămân în voi cuvintele Mele, cereți orice veți vrea, și vi se va da. Dacă aduceți mult rod, prin aceasta Tatăl Meu va fi proslăvit; și voi veți fi astfel ucenicii Mei. Cum M-a iubit pe Mine Tatăl, așa v-am iubit și Eu pe voi. Rămâneți în dragostea Mea. Dacă păziți poruncile Mele, veți rămâne în dragostea Mea, după cum și Eu am păzit poruncile Tatălui Meu și rămân în dragostea Lui. V-am spus aceste lucruri, pentru ca bucuria Mea să rămână în voi, și bucuria voastră să fie deplină.",
      teaching:
        "Să luăm bine seama la marele cuvânt al capitolului: rămâneți. Domnul nu cere de la ucenici să inventeze viața, ci să rămână în Izvorul ei. Mlădița nu produce viață din ea însăși și nici nu se laudă cu rodul ca și cum ar fi al ei. Totul vine din legătura vie cu Vița.\n\nApoi se cuvine să luăm aminte la curățirea făcută de Vier. Rodirea nu înseamnă lipsa tăierilor și a curățirii, ci tocmai trecerea prin ele. Tatăl nu curăță ca să piardă mlădița roditoare, ci ca să o facă și mai roditoare. Multe dureri ale ucenicului nu sunt semn că a fost părăsit, ci că este lucrat.\n\nȘi iarăși, despărțiți de Mine nu puteți face nimic. Câtă simplitate și cât adevăr stau aici! Nu se spune: nu puteți face puțin bine, ci nimic. Toată viața roditoare a credinciosului stă atârnată de Hristos. De aceea rugăciunea, ascultarea și bucuria nu se nasc din auto-suficiență, ci din rămânere.",
      words: [
        {
          original: "μένω",
          transliteration: "meno",
          language: "greaca",
          meaning:
            "a rămâne. A sta statornic, viu și legat de Hristos."
        },
        {
          original: "καθαίρω",
          transliteration: "kathairo",
          language: "greaca",
          meaning:
            "a curăța, a tăia spre curățire. Lucrarea Tatălui asupra mlădiței roditoare."
        },
        {
          original: "καρπός",
          transliteration: "karpos",
          language: "greaca",
          meaning:
            "rod. Viață și lucrare născute din unirea cu Hristos."
        }
      ],
      crossRefs: [
        "Psalmul 80:8-9",
        "Isaia 5:1-7",
        "Ioan 14:23",
        "Galateni 5:22-23"
      ],
      forYourHeart:
        "Dacă simți uscăciune, nu începe cu rodul, ci cu Vița. Întoarce-te la Hristos, rămâi în cuvântul Lui și nu disprețui tăierile prin care Tatăl vrea să te facă mai roditor."
    },
    {
      id: "ioan-15-12-17",
      ref: "Ioan 15:12-17",
      heading: "Să vă iubiți cum v-am iubit Eu",
      text:
        "Aceasta este porunca Mea: să vă iubiți unii pe alții, cum v-am iubit Eu. Nu este mai mare dragoste decât să-și dea cineva viața pentru prietenii săi. Voi sunteți prietenii Mei, dacă faceți ce vă poruncesc Eu. Nu vă mai numesc robi, pentru că robul nu știe ce face stăpânul său; ci v-am numit prieteni, pentru că v-am făcut cunoscut tot ce am auzit de la Tatăl Meu. Nu voi M-ați ales pe Mine; ci Eu v-am ales pe voi; și v-am rânduit să mergeți și să aduceți rod, și roada voastră să rămână, pentru ca orice veți cere de la Tatăl, în Numele Meu, să vă dea. Vă poruncesc aceste lucruri, ca să vă iubiți unii pe alții.",
      teaching:
        "Să luăm bine seama la măsura iubirii cerute: cum v-am iubit Eu. Domnul nu lasă dragostea frățească la nivelul simpatiei, al afinității sau al schimbului de bunăvoințe. Măsura ei este dragostea Lui jertfitoare. De aceea porunca aceasta nu poate fi împlinită din fire, ci numai din rămânere în El.\n\nApoi, numele de prieteni nu slăbește autoritatea Domnului, ci adâncește apropierea. El nu încetează să fie Domnul, dar face cunoscută inima Tatălui celor ai Lui. În prietenia aceasta nu intri prin egalitate cu Hristos, ci prin harul Lui care te trage aproape și te face părtaș la cuvântul Său.\n\nȘi iarăși, nu voi M-ați ales pe Mine; ci Eu v-am ales pe voi. Câtă smerenie ar trebui să nască acest cuvânt! Rodirea nu stă pe temelia inițiativei noastre, ci pe alegerea și rânduirea Lui. De aceea orice slujire curată trebuie să rămână fără laudă în sine.",
      words: [
        {
          original: "φίλοι",
          transliteration: "philoi",
          language: "greaca",
          meaning:
            "prieteni. Cei apropiați cărora Domnul le face cunoscut ce a auzit de la Tatăl."
        },
        {
          original: "τίθημι τὴν ψυχήν",
          transliteration: "tithemi ten psychen",
          language: "greaca",
          meaning:
            "a-și pune viața. Limbaj al jertfirii de sine."
        }
      ],
      crossRefs: [
        "Ioan 13:34-35",
        "Ioan 10:11",
        "Romani 5:8",
        "1 Ioan 3:16-18"
      ],
      forYourHeart:
        "Când dragostea față de frați ți se pare grea, uită-te iarăși la felul în care te-a iubit Hristos. Porunca Lui nu te trimite la rezervele firii tale, ci la crucea Lui."
    },
    {
      id: "ioan-15-18-27",
      ref: "Ioan 15:18-27",
      heading: "Dacă vă urăște lumea",
      text:
        "Dacă vă urăște lumea, știți că pe Mine M-a urât înaintea voastră. Dacă ați fi din lume, lumea ar iubi ce este al ei; dar, pentru că nu sunteți din lume și pentru că Eu v-am ales din mijlocul lumii, de aceea vă urăște lumea. Aduceți-vă aminte de vorba pe care v-am spus-o: ‘Robul nu este mai mare decât stăpânul său.’ Dacă M-au prigonit pe Mine, și pe voi vă vor prigoni; dacă au păzit cuvântul Meu, și pe al vostru îl vor păzi. Dar vă vor face toate aceste lucruri pentru Numele Meu, pentru că ei nu cunosc pe Cel ce M-a trimis. Dacă n-aș fi venit și nu le-aș fi vorbit, n-ar avea păcat, dar acum n-au nicio dezvinovățire pentru păcatul lor. Cine Mă urăște pe Mine urăște și pe Tatăl Meu. Dacă n-aș fi făcut între ei lucrări pe care nimeni altul nu le-a făcut, n-ar avea păcat; dar acum le-au și văzut, și M-au urât și pe Mine și pe Tatăl Meu. Dar lucrul acesta s-a întâmplat ca să se împlinească vorba scrisă în Legea lor: ‘M-au urât fără temei.’ Când va veni Mângâietorul, pe care-L voi trimite de la Tatăl, adică Duhul adevărului, care purcede de la Tatăl, El va mărturisi despre Mine. Și voi, de asemenea, veți mărturisi, pentru că ați fost cu Mine de la început.",
      teaching:
        "Să luăm bine seama la realismul Domnului. El nu promite ucenicilor că rămânerea în El îi va face iubiți de lume. Dimpotrivă, îi pregătește pentru ură. Asta ne păzește de un creștinism care își măsoară adevărul după cât de bine este primit de toți.\n\nȘi totuși, în mijlocul urii, Domnul nu lasă ucenicii singuri. Mângâietorul va mărturisi despre El și ucenicii vor mărturisi împreună cu Duhul. Cu alte cuvinte, mărturia Bisericii nu este purtare omenească izolată, ci lucrare în care Duhul adevărului merge înainte.\n\nApoi, ura lumii este legată de necunoașterea Tatălui și de refuzul luminii primite. Se cuvine să luăm aminte că opoziția față de Hristos nu este doar diferență de gust, ci răspuns moral și duhovnicesc față de adevăr.",
      words: [
        {
          original: "κόσμος",
          transliteration: "kosmos",
          language: "greaca",
          meaning:
            "lume. Omenirea organizată în răzvrătire față de Dumnezeu."
        },
        {
          original: "μισέω",
          transliteration: "miseo",
          language: "greaca",
          meaning:
            "a urî. Împotrivirea lumii față de Hristos și ai Lui."
        },
        {
          original: "μαρτυρέω",
          transliteration: "martyreo",
          language: "greaca",
          meaning:
            "a mărturisi. Lucrarea Duhului și a ucenicilor despre Hristos."
        }
      ],
      crossRefs: [
        "Psalmul 69:4",
        "Ioan 14:26",
        "Ioan 16:1-4",
        "1 Petru 4:12-14"
      ],
      forYourHeart:
        "Nu te mira dacă mersul cu Hristos te așază uneori în răceala lumii. Dar nici nu purta mărturia singur. Duhul adevărului merge cu tine și mărturisește împreună cu tine despre Fiul."
    }
  ],
  prayer:
    "Doamne Isuse, adevărata Viță, ține-ne legați de Tine. Nu lăsa să căutăm rodul în afara rămânerii și nici să ne închipuim că putem face ceva despărțiți de Tine.\n\nCurăță-ne Tu, chiar și prin tăieri dureroase, ca să aducem rod spre slava Tatălui. Pune în noi dragostea Ta, ca să ne iubim unii pe alții nu după fire, ci după măsura jertfei Tale.\n\nȘi când vine ura lumii, întărește-ne prin Duhul adevărului, ca să rămânem statornici în mărturia despre Tine. Amin."
}
