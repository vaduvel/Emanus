...
> **Stare la 2 august 2026.** Înlocuirea ortografiei s-a făcut deja: vezi
> `docs/38-inlocuirea-textului-rccv.md`. Geneza (320 de unităţi), Matei (1071 de
> versete) şi Ioan (65 de unităţi) stau acum pe RCCV. Ce rămâne din documentul
> acesta este **partea a doua**: textul curat pentru cărţile şi capitolele la care
> încă n-am ajuns cu explicaţia.

---

## 1. Pricina: ortografia din 1924 nu se mai citeşte

Până la înlocuire, textul biblic era în ortografia ediţiei originale din 1924:
...
*s'a*, *cari*, *întunerec* — se opreşte la a treia propoziţie. O spunem pe faţă:
ortografia aceea a fost o greşeală de-a noastră, nu o credincioşie.

---

## 2. Ediţia aleasă: RCCV

**Romanian Corrected Cornilescu Version** (RCCV, 2013) — acelaşi Cornilescu, cu
ortografia adusă la zi. Nu e traducere nouă; sunt aceleaşi cuvinte:
...
**Lucrul cel mai bun:** învăţătura scrisă până acum a rămas întreagă. Cuvintele
ebraice şi greceşti, trimiterile, „Pentru inima ta" — toate stau pe aceleaşi
cuvinte româneşti.
...
De îndreptat: `docs/21-biblia-explicata.md` şi comentariul din `types.ts` spun că
RCCV este în domeniul public. Nu este dovedit pentru România.
...
Acelaşi izvor a fost folosit şi la înlocuirea din `docs/38`, deci textul nou şi cel
vechi vor fi din aceeaşi fântână.
...
Pildă: `marcu.json`, `luca.json`, `exodul.json`. Numele fişierului este acelaşi cu
`id`-ul cărţii.
...
```json
{
  ...
  "chapters": [
    [
      ...
      "După cum este scris în prorocul Isaia..."
    ]
  ]
}
```
...
1. **Virgulă, nu sedilă.** RCCV vine cu ş şi ţ (sedilă, U+015F şi U+0163). Se schimbă
   în ș şi ț (virgulă dedesubt, U+0219 şi U+021B), şi la fel majusculele. Aceeaşi
   regulă se aplică şi textului deja înlocuit din `docs/38`, unde eşantioanele arată
   încă sedile.
...
- **Capitolul explicat** — `BibleChapter` cu unităţile lui, cum îl avem la Geneza,
  Matei şi Ioan. Textul stă în unitate, lângă învăţătură. Nu se atinge.
...
câte una. Acest strat rezolvă şi neajunsul de acum: Matei şi Ioan sunt scrise, dar
toate capitolele stau `in_review`, deci cititorul nu vede nimic la ele. Cu textul
simplu, vede măcar Scriptura.
