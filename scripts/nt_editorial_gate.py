...
def _greek_source_lock_for_target(
    source_data: Mapping[str, Any],
    book_id: str,
    chapter: int,
    verse: int,
) -> str:
    """Use SBLGNT first and a declared Greek witness only for its lacunae."""
    ...
    for lock_id in candidates:
        references = source_data["source_references_for_target"](
            lock_id, book_id, chapter, verse
        )
        texts = source_data["texts"].get(lock_id, {})
        if all(isinstance(texts.get(reference), str) and texts.get(reference) for reference in references):
            return lock_id
    fail(f"{book_id}.{chapter}.{verse}: niciun martor grec fixat nu are text")
...
def validate_nt_editorial_approval(
    ...
    for reference in sorted(expected_references.intersection(by_reference), key=lambda item: _reference_sort_key(item, source_data)):
        ...
        book = source_data["books"][book_id]
        try:
            greek_lock_id = _greek_source_lock_for_target(
                source_data, book_id, chapter, verse
            )
            _validate_locked_source_evidence(
                sources.get("sblgnt"), owner + ".sources.sblgnt", source_data,
                greek_lock_id, book_id, chapter, verse,
            )
            _validate_locked_source_evidence(
            ...
            greek_refs, greek_digest, greek_missing = _expected_locked_evidence(
                source_data, greek_lock_id, book_id, chapter, verse
            )
            if greek_digest is None or greek_missing:
                fail(f"{owner}.sources.sblgnt: textul grecesc fixat lipsește")
            greek_text = "\n".join(
                source_data["texts"][greek_lock_id][tuple(map(int, item.split(":")))]
