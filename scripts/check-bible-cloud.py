...
MIGRATION = ROOT / "supabase/migrations/20260802180000_bible_product.sql"
...
def check_source() -> None:
    ...
    required = (
        "bible_chapters_published_read",
        "bible_units_published_read",
    ...
    for marker in required:
        if marker not in migration:
            fail(f"lipseste bariera SQL: {marker}")
    ...
    if "pnpm check:bible-source" not in workflow:
        fail("CI nu valideaza structura sursei Bibliei")
    ...
    print("Biblia cloud: schema, RLS, secrete si rute verificate.")
...
def check_bundle() -> None:
    ...
    # Folosim ID-uri editoriale, nu fragmente biblice care pot aparea legitim
    # intr-un curs din biblioteca.
