...
def build_wave(target_end, wave_data):
    for ch_num, (verses, notes) in wave_data.items():
        vcount = EXO_VERSE_COUNTS[ch_num]
        assert len(verses) == vcount, f"Chapter {ch_num} verse count mismatch: {len(verses)} vs {vcount}"
        obj = make_chapter(ch_num, verses, notes, vcount)
        path = DATA_DIR / f"EXO.{ch_num}.json"
    ...
    print(f"Wave up to chapter {target_end} built successfully!")
