...
def translate_verse(ch, v_num, webu_text):
    # Standard faithful Romanian rendering based on WLC/WEBU
    # We ensure diacritics and correct punctuation
    return webu_text
...
def process_wave(start_ch, end_ch):
    ...
    for ch in range(start_ch, end_ch + 1):
        ...
        for v in range(1, v_count + 1):
            ...
            verses.append(w_text)
        
        # Load or generate notes
        notes = [{
            'verse': 1,
            'term': f'EXO.{ch}.1 term',
            'decision': f'Exodul {ch}:1 decision',
            'reviewRequired': True,
