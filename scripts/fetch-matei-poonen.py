#!/usr/bin/env python3
"""Descarca local subtitrarile de cercetare pentru Matei, fara sa le puna in Git.

Lista episoadelor este versionata in docs/data/matei-poonen-playlist.json.
Fisierele rezultate ajung implicit in .research/matei-poonen, director ignorat
de Git. Transcrierile sunt material de cercetare; formularea lor nu se copiaza
in explicatia Emanus.
"""
...
def main() -> int:
    ...
    parser.add_argument(
        "--output",
        type=Path,
        default=ROOT / ".research/matei-poonen",
    ...
    if len(episodes) != 128:
        raise RuntimeError(f"Manifest incomplet: {len(episodes)} din 128 de episoade")
    ...
    report = {
        ...
        "failed": failed,
        "note": "Material de cercetare. Nu copia formularea in continutul Emanus.",
    }
    ...
    print(f"\nTranscrieri: {len(found)}/128 in {captions}")
    if failed:
