...
MANIFEST = ROOT / "docs/data/%(carte)s-poonen-playlist.json"
...
def main() -> int:
    ...
    parser.add_argument("carte", help="Numele cartii (ex: marcu, matei, ioan).")
    parser.add_argument(
        "--output",
        type=Path,
        default=None,
    ...
    asteptate = manifest.get("episodeCount", len(episodes))
    if len(episodes) != asteptate:
        raise RuntimeError(f"Manifest incomplet: {len(episodes)} din {asteptate} de episoade")
    ...
    print(f"\nTranscrieri: {len(found)}/{asteptate} in {captions}")
    if failed:
