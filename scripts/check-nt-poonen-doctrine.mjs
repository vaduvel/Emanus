...
check("apocalipsa", 13, (t, l) => {
  ...
  anyOf(t, [
    /biserica.*nu trebuie.*presupun.*(?:antihrist|fiara).*nu.*(?:ved|intaln)/,
    /fiara.*antihrist.*biserica.*(?:ved|pregat)/,
    /sfint.*(?:persever|credint).*fiara/,
    /credincios.*(?:pregatit|loial).*antihrist/,
  ], l)
})
