...
const PRE_EDIT_NORMALIZATION = {
  unitId: "1-ioan-1-5-10-source-first",
  frozenDraftSha256: "sha256:8953a84dcf67e7ee5440ac4f1fb2b56de1db72e04ec39a23b6571bd40ddbde78",
  deterministicRecoverySha256: "sha256:e64e440978778cb7e3558f0e2ec2f5114bc1626622c701ffc259f821e020d03c",
}
...
// The frozen draft for 1:5-10 predates one deterministic Romanian reader-copy
// normalization in recovery. The diagnostic run proved that this is the only
// pre-edit drift across all 14 units. Rebind the spec only from the exact old
// frozen hash to the exact recovery hash; any other value remains a hard error.
const normalizedSpec = review[PRE_EDIT_NORMALIZATION.unitId]
if (!normalizedSpec) fail(`${PRE_EDIT_NORMALIZATION.unitId}: frozen review spec missing`)
if (normalizedSpec.expectedCurrentSnapshotSha256 === PRE_EDIT_NORMALIZATION.frozenDraftSha256) {
  normalizedSpec.expectedCurrentSnapshotSha256 = PRE_EDIT_NORMALIZATION.deterministicRecoverySha256
  fs.writeFileSync(SPEC, JSON.stringify(review, null, 2) + "\n", "utf8")
  console.log(`1 Ioan pre-edit snapshot rebound after deterministic Romanian normalization: ${PRE_EDIT_NORMALIZATION.unitId}.`)
} else if (normalizedSpec.expectedCurrentSnapshotSha256 !== PRE_EDIT_NORMALIZATION.deterministicRecoverySha256) {
  fail(`${PRE_EDIT_NORMALIZATION.unitId}: unexpected frozen snapshot hash ${normalizedSpec.expectedCurrentSnapshotSha256}`)
}
