...
let addressableWaveReady = true
const romansSnapshotFix = spawnSync("python3", ["scripts/fix_nt_addressable_wave1_romans_spec.py"], { stdio: "inherit" })
if (romansSnapshotFix.status === 42) {
  addressableWaveReady = false
  console.log("Addressable semantic wave 1: Romans approval deferred until reviewed-vs-current drift is manually reconciled.")
} else if (romansSnapshotFix.status !== 0) {
