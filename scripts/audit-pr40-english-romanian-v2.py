#!/usr/bin/env python3
"""Audit English-source Romanian candidates with source-exact versification."""
...
def repetition_detected(value: str) -> bool:
    words = re.findall(r"\w+", value.lower(), flags=re.UNICODE)
    if len(words) < 12:
        return False
    bigrams = list(zip(words, words[1:]))
    return len(set(bigrams)) / max(len(bigrams), 1) < 0.45
...
def load_deuterocanon_sources() -> tuple[dict[tuple[str, int, int], str], dict[str, dict[tuple[str, int, int], str]]]:
    ...
    spec = importlib.util.spec_from_file_location("deut_source_parser_v2", script)
...
def greek_coverage(
    ...
    aliases = GREEK_BOOK_ALIASES.get(book_id, {book_id})
    exact_refs = {(book_id, chapter, verse)}
    if book_id == "BAR" and chapter == 6:
        exact_refs.add(("LJE", 1, verse))
    if book_id == "PS2":
        exact_refs.update({("PS2", 1, verse), ("PSA", 151, verse)})
    coverage: dict[str, bool] = {}
    book_level = False
    for name, witness in witnesses.items():
        coverage[name] = any(ref in witness for ref in exact_refs)
        if any(ref[0] in aliases for ref in witness):
            book_level = True
    return coverage, book_level
...
def main() -> None:
    ...
    for path in candidate_paths:
        ...
        chapter_key = (document["bookId"], int(document["chapter"]))
        expected_numbers = sorted(
            ref[2] for ref in source_map
            if ref[0] == chapter_key[0] and ref[1] == chapter_key[1]
        )
        numbers = [int(verse["number"]) for verse in document.get("verses", [])]
        if numbers != expected_numbers:
            raise RuntimeError(
                f"{path.name}: candidate numbering differs from pinned source; "
                f"expected={expected_numbers[:12]}... actual={numbers[:12]}..."
            )
        for verse in document.get("verses", []):
    ...
    for row, score in zip(rows, scores):
        ...
        if args.collection == "deuterocanon":
            coverage, book_level = greek_coverage(
                row["bookId"], row["chapter"], row["verse"], greek_witnesses
            )
                    ...
                    warnings.append(
                        {
                            ...
                            "code": "GREEK_WITNESS_VERSIFICATION_DIFFERS",
                            "coverage": coverage,
                        }
                    )
                else:
                    codes.append("NO_GREEK_BOOK_WITNESS")
        ...
        document.setdefault("audit", {}).update(
            {
                ...
                "versificationPolicy": "candidate coordinates must equal the pinned source coordinates exactly",
            }
        )
