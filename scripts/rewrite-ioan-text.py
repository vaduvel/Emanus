#!/usr/bin/env python3
"""Inlocuieste textul biblic din unitatile Ioanului cu textul din sursa publica.

Aceeasi sursa si aceeasi randuiala ca rewrite-bible-text.py pentru Geneza:
wordproject.org, editia Cornilescu 1924 cu ortografia ei.

Atinge numai campul `text` al fiecarei unitati. Explicatiile, cuvintele
greiesti, trimiterile si rugaciunile raman neatinse.
...
SURSA = "https://www.wordproject.org/bibles/ro/43/%d.htm"
BIBLE = Path("packages/shared/src/bible")
RAPORT = Path("docs/37-inlocuirea-textului-ioan.md")
APLICA = "--proba" not in sys.argv

UNITATE = re.compile(
    r'(id:\s*"ioan-(\d+)-(\d+)-(\d+)".*?text:\s*")((?:[^"\\]|\\.)*)(")',
    re.DOTALL,
)
...
def main():
    ...
    for capitol in range(1, 22):
        ...
        cale = BIBLE / nume
        if not cale.exists():
            sarite.append("Ioan %d: lipseste fisierul." % capitol)
            continue
        try:
            versete = adu_capitolul(capitol)
        except Exception as eroare:  # noqa: BLE001
            sarite.append("Ioan %d: sursa nu a putut fi adusa (%s)." % (capitol, eroare))
            continue
        ...
        def inlocuieste(potrivire):
            ...
            cap, de_la, pana_la = int(cap), int(de_la), int(pana_la)
            if cap != capitol:
                return potrivire.group(0)
            bucati = [versete.get(v, "").strip() for v in range(de_la, pana_la + 1)]
            if not all(bucati):
                sarite.append(
                    "Ioan %s:%d-%d: versetele nu s-au gasit intregi in sursa."
                    % (cap, de_la, pana_la)
                )
                return potrivire.group(0)
            nou = imbraca(" ".join(bucati))
            if len(nou) < 20:
                sarite.append("Ioan %s:%d-%d: textul adus e prea scurt." % (cap, de_la, pana_la))
                return potrivire.group(0)
    ...
    RAPORT.write_text(
        "# Inlocuirea textului biblic Ioan din sursa\n\n"
        + (
            "Textul biblic al tuturor unitatilor Ioanului a fost adus din editia\n"
