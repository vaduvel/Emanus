#!/usr/bin/env python3
"""Audit one selected Qumran witness shard against the full pinned source corpus."""
    ...
    for witness in sorted(candidates):
        ...
        for key in sorted(source_lines):
            ...
            if source.get("isTotalLacuna"):
                ...
                group = grouped.setdefault(key[0], {"source": [], "target": [], "aramaic": []})
                group["source"].append(str(source.get("normalized") or source.get("diplomatic") or ""))
                group["target"].append(romanian)
                if source.get("language") == "a":
                    group["aramaic"].append(reference)
                    warnings.append(
                        {
                            "reference": reference,
