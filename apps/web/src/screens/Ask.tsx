...
export function Ask() {
    ...
    const paths = manifest.paths.map((path) => ({
      ...
      searchText: normalize(
        [
          path.title,
          path.promise,
          ...path.lessons.map((lesson) => lesson.title),
          PATH_SEARCH_ALIASES[path.id] ?? "",
        ].join(" "),
      ),
    }))
    const courses = visibleContentShelves()
  ...
  const results = useMemo(() => {
    ...
    return resources
      .filter(
        (resource) =>
          resource.searchText.includes(term) ||
          (words.length > 0 &&
            words.every((word) => resource.searchText.includes(word))),
      )
  ...
  return (
    <section className="ask-screen">
      ...
      <p className="muted">
        Scrie întrebarea în cuvintele tale. Îți arătăm numai răspunsuri din
        cursurile verificate; când nu avem unul, nu inventăm.
      </p>
      <label className="ask-screen__search">
        ...
        <div>
          ...
          <input
            ...
            placeholder="De exemplu: cum pot ierta?"
          />
        </div>
      </label>

      {!query ? (
        <div className="ask-screen__suggestions" aria-label="Întrebări frecvente">
      ...
      {query && results.length > 0 ? (
        <div className="ask-screen__results">
          <p className="today__kicker">Răspunsuri găsite</p>
          {results.slice(0, 12).map((resource) => (
            <button
              key={resource.id}
              type="button"
              className="tile ask-result"
              disabled={resource.lessonIds.length === 0}
              onClick={() => openCourse(resource)}
            >
              <BookOpen size={19} aria-hidden />
              <span>
                <strong>{resource.title}</strong>
                <small>{resource.forWhom}</small>
              </span>
            </button>
          ))}
        </div>
      ...
      {query && results.length === 0 ? (
        <div className="tile ask-screen__empty">
          <h2>Nu avem încă un răspuns verificat</h2>
          <p className="muted">
            Păstrăm întrebarea în câmpul tău, dar nu o trimitem nimănui și nu
            generăm automat un răspuns spiritual.
          </p>
        </div>
      ) : null}
    </section>
