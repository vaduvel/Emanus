...
const DRUG_PATTERNS = [
  ...DRUG_EMERGENCY_PATTERNS,
  /\bdependen(?:t|ta|te) de droguri\b/,
  /\bconsum(?:at|a|ul)? droguri\b/,
  /\bdroguri\b/,
  /\bheroina\b/,
  /\bcocaina\b/,
]
...
export function detectCrisisIntents(input: string): CrisisIntent[] {
  ...
  if (matchesAny(value, DRUG_PATTERNS)) intents.push("drugs")
  if (matchesAny(value, GAMBLING_PATTERNS)) intents.push("gambling")
